...
//    private Resource keystoreResource = null;
     ...
     */
    public SamlGenerator(Resource keystoreResource, org.apache.commons.configuration.Configuration configuration) throws ConfigurationException, ClassNotFoundException {
        this(new CommonsConfiguration(configuration));
        log.info("SamlGenerator: This line is being reached");
    }
     ...
     * @throws ConfigurationException 
     */
    public SamlGenerator(com.intel.mtwilson.util.configuration.Configuration configuration /*Resource keystoreResource, org.apache.commons.configuration.Configuration configuration*/) throws ConfigurationException, ClassNotFoundException {
        builderFactory = getSAMLBuilder();
        
        if(builderFactory != null) log.info("builderFactory is not null"); else log.info("builderFactory is null");
        
        try {
            signatureGenerator = new SAMLSignature(/*keystoreResource,*/ configuration);
            
            if(signatureGenerator != null) log.info("signatureGenerator is not null"); else log.info("signatureGenerator is null");
            
        } catch (KeyStoreException | NoSuchAlgorithmException | UnrecoverableEntryException | IllegalAccessException | InstantiationException | IOException | CertificateException ex) {
