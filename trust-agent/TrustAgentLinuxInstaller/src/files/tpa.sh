...
tpmnvinfo=tpm_nvinfo
tpmnvdefine=tpm_nvdefine
tpmnvwrite=tpm_nvwrite
tpmnvrelease=tpm_nvrelease
tpmnvread=tpm_nvread
tpmtakeownership=tpm_takeownership
tpmclear=tpm_clear
...
function getLocalTag() {
 ...
 if [ -f "$XML_FILE_LOCATION" ]; then
   ...
   tagFile=$(dialog --stdout --backtitle "$TITLE" --stdout --title "Please select your xml with Asset Tag Info" --fselect ~ 14 48)
   if [ $? -eq 1 ]; then 
     functionReturn=1
     return
   fi
...
#    json='{"selections":[{"name":"'$selectionName'"}]}'
 ...
 if [ $resp -eq 0 ]; then
  ...
  echo "Using nvrampass: $nvramPass"
